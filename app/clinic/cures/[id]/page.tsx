...
export default function CURESDetailPage() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <Link href="/clinic/cures" className="inline-flex items-center gap-1.5 text-sm text-graphite-500 hover:text-white transition-colors">
        <ArrowLeft className="w-4 h-4" /> Back
      </Link>
      <div className="flex items-center gap-3">
        <Shield className="w-5 h-5 text-graphite-500" />
      ...
      <div className="grid grid-cols-2 gap-4">
        {[['Patient', data.patient_id?.slice(0, 12) + '...'], ['Method', data.query_method], ['Query Date', new Date(data.query_datetime).toLocaleString()], ['Risk', data.risk_stratification], ['Outage', data.outage_attested ? 'Yes' : 'No']].map(([l, v]) => (
          <div key={l as string} className="p-3 rounded-lg bg-graphite-800/30">
            <p className="text-[10px] text-graphite-500 uppercase">{l}</p>
      ...
      {data.clinical_judgment_text && (
        <div className="p-4 rounded-lg bg-graphite-800/30">
          <p className="text-[10px] text-graphite-500 uppercase mb-2">Clinical Judgment</p>
