    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">California PDMP queries for Schedule III TRT prescriptions</p>
        </div>
      ...
      <div className="rounded-xl border border-zinc-800 overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-sm text-zinc-500">No CURES attestations found.</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead><tr className="bg-zinc-900/80 border-b border-zinc-800">
              ...
              <th className="text-left px-4 py-3 text-xs font-medium text-zinc-500 uppercase">Risk</th>
            </tr></thead>
            <tbody className="divide-y divide-zinc-800/50">
              {queries.map(q => (
                <tr key={q.id} className="hover:bg-zinc-900/50">
                  <td className="px-4 py-3 text-zinc-400">{q.patient_id?.slice(0, 8)}...</td>
                  <td className="px-4 py-3 text-white">{q.query_method}</td>
                  <td className="px-4 py-3 text-zinc-400">{new Date(q.query_datetime).toLocaleDateString()}</td>
