    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">Manage clinical and operational tasks</p>
        </div>
      </div>
          ...
          {([] as any[]).map(refill => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <Link href={`/clinic/patients/${refill.patientId}`} className="text-xs text-zinc-400 hover:text-nova-400 transition-colors">
                  {refill.patientName}
                </Link>
              </div>
              <div className="text-right">
                ...
                <p className="text-[10px] text-zinc-600 mt-0.5">{refill.requestedAt}</p>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    ...
    >
      ...
      <span className={cn(
        ...
        active ? 'bg-nova-500/20 text-nova-400' : 'bg-zinc-800 text-zinc-500'
      )}>{count}</span>
    </button>
  )
}
...
function PriorityIndicator({ priority }: { priority: TaskPriority }) {
  const styles: Record<TaskPriority, string> = {
    urgent: 'bg-red-400', high: 'bg-orange-400', normal: 'bg-blue-400', low: 'bg-zinc-400',
  }
...
function TypeBadge({ type }: { type: TaskType }) {
  ...
  return (
    <span className="text-[10px] px-1.5 py-0.5 rounded bg-zinc-800 text-zinc-400 border border-zinc-700 capitalize">
      {labels[type]}
    </span>
  )
}
...
function RefillStatusIcon({ status }: { status: string }) {
  const color = status === 'pending' ? 'text-yellow-400 bg-yellow-500/10' :
    ...
    status === 'shipped' ? 'text-blue-400 bg-blue-500/10' : 'text-zinc-400 bg-zinc-500/10'
  return (
