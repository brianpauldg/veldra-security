...
export default function JustificationDetailPage() {
  ...
  if (loading) return <div className="text-graphite-500 py-8">Loading...</div>
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="flex items-center gap-3">
        <FileText className="w-5 h-5 text-graphite-500" />
      ...
      {data.status === 'flagged' && (
        <div className="rounded-lg border border-red-500/20 bg-red-500/5 p-4 flex items-start gap-2">
          ...
          <div><p className="text-xs text-red-400 font-medium">Flagged: {data.flagged_reason}</p>
            <p className="text-[10px] text-graphite-500 mt-1">Flagged at {data.flagged_at ? new Date(data.flagged_at).toLocaleString() : 'N/A'}</p></div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-4">
        {[
          ...
          <div key={label as string} className="p-3 rounded-lg bg-graphite-800/30">
            <p className="text-[10px] text-graphite-500 uppercase tracking-wider">{label}</p>
            <p className="text-xs text-white mt-1">{value}</p>
          </div>
        ))}
      </div>

      <div className="p-4 rounded-lg bg-graphite-800/30">
        <p className="text-[10px] text-graphite-500 uppercase tracking-wider mb-2">Deviation Categories</p>
        <p className="text-xs text-white">{JSON.stringify(data.deviation_categories)}</p>
        {data.deviation_details && <p className="text-xs text-graphite-400 mt-2">{data.deviation_details}</p>}
      </div>

      <div className="p-4 rounded-lg bg-graphite-800/30">
        <p className="text-[10px] text-graphite-500 uppercase tracking-wider mb-2">Clinical Rationale</p>
